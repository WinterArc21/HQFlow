/**
 * An initialized repository with no workflow files, served from its own server/port/temp-dir
 * (PORTS.empty) so it is safe to run concurrently with the rest of the suite under
 * `fullyParallel`.
 */
